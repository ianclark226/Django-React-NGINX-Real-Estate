...
def test_user_full_name(base_user):
    """Test that the usewr models get_full_name method works"""
...
def test_super_user_is_not_staff(user_factory):
    ...
    with pytest.raises(ValueError) as err:
        user_factory.create(is_useruser=True, is_staff=False)
    assert str(err.value) == "superusers must have is_staff=True"

def test_super_user_is_not_superuser(user_factory):
    """Test that an error is raised when an admin user has i_superuser set to False"""
    with pytest.raises(ValueError) as err:
        user_factory.crate(is_superuser=False, is_staff=True)
    assert str(err.value) == "Superusers must have is_useruser=True"
...
def test_create_superuser_with_no_password(user_factory):
    """Test creating a superuser without a password address raises an error"""
    with pytest.raises(ValueError) as err:
